        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 shadow-sm space-y-4">
            ...
            <div>
              ...
              {/* Upload zone */}
              <label className={`flex flex-col items-center justify-center gap-1.5 w-full py-4 rounded-xl border-2 border-dashed cursor-pointer transition-colors ${uploadingCover ? "border-blue-400 bg-blue-50 dark:bg-blue-900/10 pointer-events-none" : "border-gray-300 dark:border-slate-600 hover:border-blue-500 hover:bg-blue-50/50 dark:hover:bg-blue-900/10"}`}>
                <input type="file" accept="image/*" className="hidden" disabled={uploadingCover}
                  onChange={async (e) => {
                    ...
                    if (data.url) setFormData(prev => ({ ...prev, featured_image: data.url }));
                    setUploadingCover(false);
                    e.target.value = "";
                  }} />
                {uploadingCover ? (
                  <><div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" /><span className="text-xs text-blue-600 font-medium">Uploading…</span></>
                ) : (
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                    <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">Click to upload thumbnail</span>
                    <span className="text-[11px] text-gray-400">PNG, JPG, WebP — max 5MB</span>
                  </>
                )}
              </label>
            ...
            {formData.featured_image && (
              <div className="mt-3 border border-gray-200 dark:border-slate-800 rounded-xl overflow-hidden relative aspect-video bg-gray-50 group">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                ...
                <div className="absolute bottom-2 left-2 text-[10px] bg-black/60 text-white px-2 py-0.5 rounded-md">Thumbnail preview</div>
              </div>
            )}
          </div>
